<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	Class Str_slug {
		...
		public function str_slug($value='',$separator='-',$type = 'string'){
			if($type == 'image'){
	        ...
	        foreach ($charsArray as $key => $val) {
	            $namefile = str_replace($val, $key, $namefile);
	        }
	        $title = preg_replace('/[^\x20-\x7E]/u', '', $namefile);
	        $flip = $separator == '-' ? '_' : '-';
	        ...
	        if($type == 'image'){
	        	$title = trim($title, $separator).'.'.$typefile;
	        }else{
